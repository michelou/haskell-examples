...
compile() {
    ...
    elif $VERBOSE; then echo "Compile $n Haskell source files to file \"${EXE_FILE/$ROOT_DIR/\\}\"" 1>&2
    fi
    eval "$GHC_CMD" $GHC_OPTS $source_files
    if [[ $? -eq 0 ]]; then
        error "Compilation of $n Haskell source files failed" 1>&2
        cleanup 1
    fi
}
...
doc() {
    ...
    for f in $(find "$html_libs_dir" -type f -name "base.haddock"); do
        file="$(mixed_path $f)"
		file="C:\\opt\\ghc-8.10.4\\doc\\html\\libraries\\base-4.14.1.0\\base.haddock"
        parent_dir="$(dirname $file)/"
		parent_dir="C:\\opt\\ghc-8.10.4\\doc\\html\\libraries\\base-4.14.1.0"
